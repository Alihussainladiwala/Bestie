...
app.post("/addToCart",(req,res) => {
    ...
    db.query("INSERT into CartProduct (product_id, cart_id ) VALUES (?,?)",[productid, cartid],(err,result) => {
        console.log(err);
        if(err){
...
app.get("/getCheckOutList/:user_id",(req,res) => {
    ...
    db.query("SELECT A.name, A.amount FROM ((MasterClassDB.Product AS A LEFT JOIN MasterClassDB.CartProduct AS B ON A.id = B.product_id) LEFT JOIN MasterClassDB.Cart AS C ON B.cart_id = C.id) WHERE C.user_id = ?  ",[user_id],(err,result) => {
        console.log(err);
        if(err){
